...
public class BinarySearchTree<T extends Comparable<T>> {
    ...
    //Add an element to this binary tree. Returns true if we succeessfully perform an insertion
    ...
    private Node add(Node node, T elem) {
        ...
        if (node == null) {
            ...
            //Place lower elements in left subtree
    ...
    private Node remove(Node node, T elem) {
        ...
        if (cmp < 0) {
            ...
            //Dig into right subtree, the value if the value we're looking for is greater than the current value
        } else if (cmp > 0) {
            ...
            if (node.left == null) {
                ...
                node = null;
                return leftChild;
                //When removing a node from a binary tree with two links the successo rof the node being removed can either be
                //the largest value in the left subtree or the smallest value in the right subtree.
                //In this implementation I have decided to find the smallest value in the right subtree which can be found by
    ...
    private boolean contains(Node node, T elem) {
        ...
        //Dig into the left subtree bc the value we're looking for is smaller than the current value
        if (cmp < 0)
            return contains(node.left, elem);
        //Dig into the right subtree bc the value we're looking for is larger than the current value
        else if (cmp > 0)
    ...
    private int height(Node node) {
        if (node == null)
            return 0;
        //so offensichtlich, wie bin ich da nicht drauf gekommen..
        return Math.max(height(node.left), height(node.right)) + 1;
    }
    
    //Meine (sind müll und klappen so nicht)
